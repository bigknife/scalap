...
class NominateServiceHandler extends NominateService.Handler[Stack] {
  ...
  override def findRoundLeaders(quorumSet: QuorumSet,
    ...
    val neighbors: Set[NodeID] = quorumSet.neighbors(round, slotIndex, previousValue)
  ...
  override def nominateNewValues(tracker: NominateTracker,
                                 nodeID: NodeID,
                                 tryToNominate: Value,
...
object NominateServiceHandler {
  private lazy val _instance: NominateServiceHandler = new NominateServiceHandler
  trait Implicits {
